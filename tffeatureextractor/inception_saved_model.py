...
def export():
    ...
    with tf.Graph().as_default():
        ...
        classes = get_classes(indices, synsets, texts)
        ...
        with tf.Session() as sess:
            restore_checkpoint(saver, sess)# Export inference model.
...
def get_signature_def_map(classes, jpegs, serialized_tf_example, values):
    ...
    classification_signature = get_classification_signature(classes_output_tensor_info,
...
def get_classes(indices, synsets, texts):
...
def get_classification_signature(classes_output_tensor_info, scores_output_tensor_info, serialized_tf_example):
...
def get_prediction_signature(classes_output_tensor_info, jpegs, scores_output_tensor_info):
...
def restore_checkpoint(saver, sess):
    ...
    if ckpt and ckpt.model_checkpoint_path:
        ...
        raise Exception('No checkpoint file found at %s' % FLAGS.checkpoint_dir)
